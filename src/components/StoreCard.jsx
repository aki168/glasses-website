...
export default function StoreCard(props) {
  ...
  // const [mapModeData, setMapModeData] = useState(props.dataCon);
  // const [click, setClick] = useState(''); 
  const path = 'https://raw.githubusercontent.com/aki168/glasses-website/main/public';
  
  console.log(props)
  const checkShop = (e) => {
    navigate('/stores/details',{state:{ selectedWhat:props.id }},{replace: true})
  //   e.preventDefault();
  //   // onItemClick(e.target.name)
  //   setMode(prevMode => !prevMode);
  //   // setMapModeData( );
  //   console.log('地圖模式的店家',mapModeData);
  ...
  return (
    ...
    // <ul className="flex flex-wrap gap-5">
      ...
      // </ul>
      // : <p>哈囉地圖{mapModeData}</p> 
      // 地圖模式
      // { mapModeData.map ( item =>  <StoreWithMap key={nanoid()} {...item} />) }
